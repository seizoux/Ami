...
class Games(commands.Cog):
    ...
    async def rockpaperscissors(self, ctx, bet:int):
        db = await self.bot.db.fetch("SELECT * FROM old_cuppy WHERE user_id = $1", str(ctx.author.id))
        if not db:
        ...
        if bot_move == move_author or move_author == bot_move:
            ...
            await self.bot.db.execute("UPDATE old_cuppy SET wallet = $1 WHERE user_id = $2", db[0]["wallet"] - bet, str(ctx.author.id))
    ...
    async def tictactoe(self, ctx, opponent: discord.Member, bet: int):
        ...
        db2 = await self.bot.db.fetch("SELECT * FROM old_cuppy WHERE user_id = $1", str(opponent.id))
        if not db2:
            return await ctx.send(
        ...
        while True:
            ...
            if msg.content.isdigit():
                ...
                if winner != 0:
                    ...
                    if winner == 1:
                        await self.bot.db.execute("UPDATE old_cuppy SET wallet = $1 WHERE user_id = $2",
