...
class Levels(commands.Cog):
    ...
    async def on_message(self, message):
        try:
            ...
            data = await self.bot.pg_con.fetchrow("SELECT * FROM levels WHERE user_id = $1 AND guild_id = $2", author,
                                                  guild)

            if data and data['xp'] >= 500000:
                await self.bot.pg_con.execute("UPDATE levels SET xp = $1 WHERE user_id = $2 AND guild_id = $3", 0,
                                              author, guild)
                ...
                try:
                    await self.bot.pg_con.execute("UPDATE users SET bank = $1 WHERE user_id = $2",
                                                  data1["bank"] + 100000, author)
                    await message.channel.send(
                        f"{message.author.name}, congrats to have reached **`500.000 xp`**! Your xp has been reset to `0` and i've sent **`100.000 coins`** in your bank as reward!")
                except Exception:
                    pass

            if not data:
                await self.bot.pg_con.execute("INSERT INTO levels (user_id, guild_id, level, xp) VALUES ($1, $2, 1, 1)",
                                              author, guild)
                return
            ...
            await self.bot.pg_con.execute("UPDATE levels SET xp = $1 WHERE user_id = $2 AND guild_id = $3",
                                          user["xp"] + number, author_id, guild_id)
    ...
    async def lvlup(self, ctx, levels=None):
        ...
        if levels:
            ...
            if bal < amo - 1:
                ...
                await ctx.send(
                    f"<:maeHeart:820992680808939520> {ctx.author.mention} goes from level **{user[0]['level']}** to level **{user[0]['level'] + int(levels)}!**")
        else:
            await ctx.send(
                f"<:maeHeart:820992680808939520> {ctx.author.mention} is now level **{user[0]['level'] + int(levels)}!**")
        ...
        await self.bot.pg_con.execute("UPDATE levels SET level = $1 WHERE user_id = $2 AND guild_id = $3",
                                      user[0]["level"] + int(levels), author_id, guild_id)

    @commands.command(
        help="Set your custom level card background! The link needs to redirect directly to the image.\nExample: ami setlevelbg https://wallpapercave.com/wp/wp6042272.jpg")
    ...
    async def lvl(self, ctx, member: discord.Member = None):
        ...
        guild_id = str(ctx.guild.id)
        data = await self.bot.pg_con.fetch("SELECT * FROM levels WHERE user_id = $1 AND guild_id = $2", author_id,
                                           guild_id)
        ...
        rank = await self.bot.pg_con.fetchval(
            "SELECT count(*)+1 FROM levels WHERE guild_id = $1 AND level > (SELECT level FROM levels WHERE guild_id = $1 AND user_id = $2)",
            guild_id, author_id)
        ...
        try:
            ...
            args = {
                'bg_image': "https://cdn.statically.io/img/wallpapercave.com/wp/wp3025493.png",
