...
class Eco(commands.Cog):
    ...
    async def buyitem(self, ctx, item=None, amount=None):
        ...
        if not data:
            return await ctx.reply("Dude, you can't buy items if you don't buy a place where store it.. use `ami buygarage` to setup your personal garage!")
        ...
        if item in items_names:
            if item == 'Water Bottle':
                ...
                if its['bank'] < price:
                    return await ctx.reply(f"You can't buy this item because you don't have **`{price}`** coins in your bank.")
                amount = amount
    ...
    async def garage(self, ctx):
        ...
        data = await self.bot.pg_con.fetchrow("SELECT * FROM items WHERE user_id = $1", author_id)
        if not data:
            return await ctx.reply("You don't have a garage, and for consequence no items. Use `ami buygarage` to can use this command.")
