...
class Eco(commands.Cog):
    ...
    async def bal(self, ctx, member: discord.User = None):
        ...
        if member:
            ...
            if not user:
                return await ctx.reply("This member doesn't have a balance.")
    ...
    async def bankrob(self, ctx, member: discord.Member):
        ...
        if not ite['computers']:
            return await ctx.reply("You don't have a fucking computer, go back to watch horny stuff.", mention_author=False)
        ...
        if chest in chests:
            if chest == "common":
                if not ite['common_chests']:
                    return await ctx.reply("You don't have <:common_chest:838028933920325633> **Common Chests** to open.", mention_author=False)
                coins = common_coins
            ...
            if chest == "rare":
                if not ite['rare_chests']:
                    return await ctx.reply("You don't have <:rare_chest:838028934598885417> **Rare Chests** to open.", mention_author=False)
                coins = rare_coins
            ...
            if chest == "epic":
                if not ite['epic_chests']:
                    return await ctx.reply("You don't have <:epic_chest:838028935479820319> **Epic Chests** to open.", mention_author=False)
                coins = epic_coins
