...
class Welcome(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        if member.guild.id == 800176902765674496:
            ...
            await channel.send(
                f'**Welcome {member.mention}! You are our `{guild.member_count}th` member! Say "hi" in** <#800935539230113803> **!** <a:blackribbon:819739315004637194> ')
            return
    ...
    @commands.command(help="Config the welcome message for the guild!")
    ...
    async def setwelc(self, ctx):
        ...
        em = discord.Embed(title="Custom welcome config!",
                           description="Hi! Here you can set your **custom welcome message** for this guild! First of all, only members with **administrator** permission can set it! If you **don't** set it, when a member join here, i send a message in the **guild system channel** (__where you get advice for server boosts for example__), and if you don't have a system channel, i don't send **any** welcome message.\nSo, if you want to set your custom welcome, click on the **reaction below**, and start the configuration! ❤",
                           color=0xffcff1)
        ...
        em = discord.Embed(title="1) The message",
                           description="What message did i need to send when someone join here? Send it (you can also use discord markdown)\n**Accepted vars:**\n```py\n{name} > will return the member name\n{member} > will return like Name#1234\n{mention} > will return the member mention\n{count} > will return the position where it joined\n{age} > will return the accout date creation\n{guild} > will return the guild name\n```",
                           color=0xffcff1)
        await msg.edit(embed=em)

        msg1 = await self.bot.wait_for('message', check=lambda
            message: message.author == ctx.author and message.channel == ctx.channel)

        if len(msg1.content) > 2000:
            ln = len(msg1.content)
            return await ctx.send(
                f"Seems you've sent a message which the lenght is {ln}. The discord limit is `2000`, so resend `ami setwelc` and try to make it shorter.")
        else:
            mex1 = str(msg1.content)
            await self.bot.pg_con.execute("UPDATE welcome SET msg = $1 WHERE guild_id = $2", mex1, guild)
            em = discord.Embed(title="2) The channel",
                               description="Where i need to send this message? Send the **`Channel ID`** (right click on the channel and `Copy ID`) (see the gif to know how to enable developer mode)",
                               color=0xffcff1)
            em.set_image(url="https://i.imgur.com/nhE8sQa.gif")
            await msg.edit(embed=em)

        msg2 = await self.bot.wait_for('message', check=lambda
            message: message.author == ctx.author and message.channel == ctx.channel)
        ...
        if msg2.content.isdigit():
            ...
            em = discord.Embed(title="3) The role",
                               description="What role i need to give to new members? Send the role name (with capital and lowercase letters, and be sure the role exist, or i don't give any role.)",
                               color=0xffcff1)
            await msg.edit(embed=em)
        else:
            return await ctx.send("This isn't a channel ID, send `ami setwelc` again and send a valid channel ID")

        msg3 = await self.bot.wait_for('message', check=lambda
            message: message.author == ctx.author and message.channel == ctx.channel)

        if msg3:
            mex3 = str(msg3.content)
            await self.bot.pg_con.execute("UPDATE welcome SET role_name = $1 WHERE guild_id = $2", mex3, guild)
            em = discord.Embed(title="4) The embed",
                               description="You want the welcome message in an embed? Send `yes` to have it, or `no` to have a normal messagee.",
                               color=0xffcff1)
            await msg.edit(embed=em)

        msg4 = await self.bot.wait_for('message', check=lambda
            message: message.author == ctx.author and message.channel == ctx.channel)

        if msg4.content in ("yes", "Yes"):
            mex4 = "on"
            await self.bot.pg_con.execute("UPDATE welcome SET embed = $1 WHERE guild_id = $2", mex4, guild)
            em = discord.Embed(title="Configuration done! Check next",
                               description="Alright! You've finished the custom welcome config!\nTo enable or disable something, use `ami welcset` and see all welcome settings!",
                               color=0xffcff1)
            await msg.edit(embed=em)
            return
    ...
    @commands.command(help="See welcome config on this server")
    async def welcset(self, ctx):
        ...
        em = discord.Embed(title="Welcome settings",
                           description="Here you can see all welcome settings for this guild!\n<:check:314349398811475968> = On\n<:xmark:314349398824058880> = Off\n<:empty:314349398723264512> = Empty",
                           color=0xffcff1)
        em.add_field(name=f"{guil.name} settings",
                     value=f"`Message` = {mex}\n`Role Name` = {rl}\n`Role Assign` = {rle}\n`Channel` = {ch}\n`Embed` = {emb}\n`Welcome` = {wel}")
        em.set_footer(text='Check welcome category in "ami help" for more info!')
        await ctx.send(embed=em)

    @commands.command(help="Set the welcome message")
    @commands.has_permissions(administrator=True)
    ...
    @commands.command(help="Set the role assigned when someone join in the server")
    @commands.has_permissions(administrator=True)
    async def setrolename(self, ctx, *, args):
        ...
        mex = str(args)
        await ctx.send("<:check:819702267476967444> **`WELCOME ROLE NAME UPDATED!`**")
        await self.bot.pg_con.execute("UPDATE welcome SET role_name = $1 WHERE guild_id = $2", mex, guild)

    @commands.command(help="Turn on/off the role assignment")
    @commands.has_permissions(administrator=True)
    ...
    @commands.command(help="Set the channel where you want to send the welcome message")
    @commands.has_permissions(administrator=True)
    ...
    @commands.command(help="Turn on/off the embed in the welcome message")
    @commands.has_permissions(administrator=True)
    ...
    @commands.command(help="Turn on/off the welcome message")
    @commands.has_permissions(administrator=True)
    ...
    async def on_guild_join(self, guild):
        try:
            website = "[website](https://amidiscord.xyz/)"
            ...
            em = discord.Embed(title=guild.name,
                               description=f"Hi cuties, thanks to have added me here! I'm ami, a multi-prupose discord bot, and i come with over 160 commands, included an entire economy system, a live music playing, and ton of other stuff!\n\nI was deployed and published on 21/01/2021, and i was online for 99.95% of the time, without issues not announced. I don't have a dashboard, i am totally discord-side, with commands to manage all my settings, but if you want, you can check my {website}, just to see some cool stuff ^^\n\nI'm able to make the experience in this discord server more enjoyable, here it is some tips to do to make me work at my best:\n\n1) Use `ami setwelc` to set a custom welcome message when someone joins here! (Tip: do it, and after use `ami wel off` to disable the welcome, or i send a message in this channel every time someone join.)\n2) Make me see all channels in this server, also without `Send Messages` permissions where you don't want me, just to logging messages and give xp to users!\n\nI hope you enojy this experience, and i appreciate if you vote me {here1} and {here2}!",
                               color=0xffcff1)
            if guild.icon_url:
    ...
    async def on_guild_remove(self, guild):
        channel = self.bot.get_channel(817439941902467103)
        em = discord.Embed(title=f"Got removed from `{guild.name}`..",
                           description=f"Lost `{guild.member_count}` members.\n\nOwner : `{guild.owner} - ({guild.owner_id})`",
                           color=0xffcff1)
        if guild.icon_url:
            em.set_thumbnail(url=guild.icon_url)
        if guild.banner_url:
