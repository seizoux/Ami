...
class Fun(commands.Cog):
    ...
    async def foo(self, message: discord.Message):
        ...
        if message.author.id in self.afks.keys():
            ...
            await message.channel.send(
                f"<:nezyay:819703490078834729> Welcome back {message.author.mention}, i removed your **AFK** status.")
            del self.afks[ids]
        ...
        if id2 in self.afks.keys():
            ...
            return await message.channel.send(
                f'<:nani:819694934491004937> {message.author.mention}, **{str(message.mentions[0].name)}** is afk for `{reasons}`')
    ...
    async def ph(self, ctx, member: discord.Member = None):
        ...
        em = discord.Embed(title="<:pepehot:819703490330492948> PornHub Simulator!",
    ...
    async def reactspeed(self, ctx, number: int):
        ...
        embed = discord.Embed(
            description=f"<:vea:819703490703523860> Your result is: `{round(end - start, 2)}` seconds", color=0xffcff1)
    ...
    async def ship(self, ctx, member: discord.Member):
        ...
        types = ["Active", "Dangerous", "Total Love", "Usless", "More hot than me", "Netflix & Bed", "Extreme",
                 "Super!"]
        types1 = random.choice(types)
        message = ctx.message
        mention1 = message.mentions[0]
        mention2 = message.mentions[1]
        em = discord.Embed(title="<:thanks:739614671774154843> Ship Results!",
                           description=f"<:gsarrow:819706480714055681> The ship » {mention1.mention} & {mention2.mention}\n\n<:gsarrow:819706480714055681> Compatibility » `{comp}/10`\n\n<:gsarrow:819706480714055681> Relation type » `{types1}`\n\n<:gsarrow:819706480714055681> Percent » `{ship}/100%`",
                           color=0xffcff1)
        em.timestamp = datetime.datetime.utcnow()
        await ctx.send(embed=em)
    ...
    async def avatar(self, ctx, member: discord.Member = None):
        ...
        em = discord.Embed(title=f"{member.name}'s avatar!",
                           description=f"{png} | {jpg} | {jpeg} | {gif} | {webp} | {tif} | {bmp}", color=0xffcff1)
    ...
    async def mc(self, ctx):
        ...
        d = discord.Embed(title=f":busts_in_silhouette: **Results:**",
                          description=f"\n<:user:819703150755577856> *Total*  »  `{a}`\n<:thumbsup:819703492481908776> *Humans*  »  `{c}`\n<:1739_CMD:819689870393999380> *Bots*  »  `{b}`",
    ...
    async def cum(self, ctx, member: discord.Member = None):
        ...
        em = discord.Embed(
            description=f"I've cummed on the **{tchoice}** of **{member.name}** at **{km}Km/h** <a:7337_Furry_Dance_Green:819689872923033631>",
            color=0xffcff1)
        await msg.edit(embed=em)
    ...
    async def time(self, ctx, member: discord.Member = None):
        ...
        if member:
            ...
            if not db1:
                return await ctx.reply(
                    f"I didn't found this member inside the database, tell him to send `ami bal` and retry again after done.")
            ...
            em = discord.Embed(title=f"<a:9932_zero_two:819689873870946395> Time for {member.name}",
                               description=f"• {t3}", color=0xffcff1)
        ...
        if not db:
            return await ctx.reply(
                f"I didn't found you inside the database, send `ami bal` to be added in the database.")
        ...
        dt_India = dt_today.astimezone(pytz.timezone(t))
        t3 = (dt_India.strftime(fmt))
        em = discord.Embed(title=f"<a:9932_zero_two:819689873870946395> Time for {ctx.author.name}",
                           description=f"• {t3}", color=0xffcff1)
    ...
    async def timezone(self, ctx, zone):
        ...
        if zone in all_timezones:
            ...
            await self.bot.pg_con.execute(
                "UPDATE users SET tz = $1 WHERE user_id = $2", zone1, author_id
            )
        ...
        while True:
            ...
            if guess.content == answer:
                ...
                await self.bot.pg_con.execute("UPDATE users SET wallet = $1 WHERE user_id = $2",
                                              user["wallet"] + 1 * earnings, author_id)
                fintime = time.time()
                total = fintime - starttime
                em = discord.Embed(
                    description=f"**{guess.author}**, well done! You got **{earnings}** coins!\nCompleted in `{round(total, 2)}` seconds!",
                    color=0x2F3136)
                await ctx.send(embed=em)
    ...
    async def sa(self, ctx, *args):
        ...
        lat = (round(self.bot.latency * 1000, 2))
        em = discord.Embed(title=f"```{search.results[0].title}```", description=search.results[0].synopsis,
                           color=0x2F3136)
    ...
    async def openchat(self, ctx):
        ...
        while True:
            try:
                ...
                await ctx.send(
                    f"**{ctx.author.name}**, ending your chat with me because you didn't send a message for `1 minute`.\nYou talked with me for **`{round(tot)} seconds`**, so you got **`{earn} coins`**, but because you ran out of time, the earn will be the half of the half of the total, so you got **`{round(earn / 4)}`** coins!")
                await self.bot.pg_con.execute("UPDATE users SET bank = $1 WHERE user_id = $2",
                                              user["bank"] + 1 * earn / 4, member_id)
            ...
            if msg.content == "closechat":
                ...
                await msg.reply(
                    f"Closing the chat, also you talked with me for **`{round(tots)} seconds`**, so you got **`{round(earn)} coins as a reward!`**")
                await self.bot.pg_con.execute("UPDATE users SET bank = $1 WHERE user_id = $2", user["bank"] + 1 * earn,
                                              member_id)
                return await cleverbot.close()
            else:
