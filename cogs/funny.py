...
class Funny(commands.Cog):
    ...
    async def slap(self, ctx, member: discord.Member):
        async with aiohttp.ClientSession() as session:
            async with session.get('https://waifu.pics/api/sfw/slap') as resp:
                d = (await resp.json())["url"]
                data = await self.bot.pg_con.fetchrow("SELECT * FROM numbers WHERE user_id = $1", str(member.id))
                if not data:
    ...
    async def kill(self, ctx, member: discord.Member):
        async with aiohttp.ClientSession() as session:
            async with session.get('https://waifu.pics/api/sfw/kill') as resp:
                d = (await resp.json())["url"]
                data = await self.bot.pg_con.fetchrow("SELECT * FROM numbers WHERE user_id = $1", str(member.id))
                if not data:
    ...
    async def kiss(self, ctx, member: discord.Member):
        async with aiohttp.ClientSession() as session:
            async with session.get('https://waifu.pics/api/sfw/kiss') as resp:
                d = (await resp.json())["url"]
                data = await self.bot.pg_con.fetchrow("SELECT * FROM numbers WHERE user_id = $1", str(member.id))
                if not data:
    ...
    async def lick(self, ctx, member: discord.Member):
        async with aiohttp.ClientSession() as session:
            async with session.get('https://waifu.pics/api/sfw/lick') as resp:
                d = (await resp.json())["url"]
                data = await self.bot.pg_con.fetchrow("SELECT * FROM numbers WHERE user_id = $1", str(member.id))
                if not data:
